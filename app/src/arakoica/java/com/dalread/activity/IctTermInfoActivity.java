...
public class IctTermInfoActivity extends BaseTermIctActivity implements View.OnClickListener  {
    ...
    private void _initLayout() {
        if (getToolbar() != null) {
            getToolbar().setTitle(R.string.detail_info);
        }
    }
    ...
    private void setEtcInfoValue() {
        StringJoiner etcInfo = new StringJoiner(",");
        if (item.isGROUP_ICT()) {
            etcInfo.add("ICT");
        }
        if (item.isGROUP_HW()) {
        ...
        if (!(TextUtils.isEmpty(item.getGROUP_NATION()))) {
            etcInfo.add(item.getGROUP_NATION());
        }
        setTextViewValue(binding.tvEtc, etcInfo.toString());
    }
