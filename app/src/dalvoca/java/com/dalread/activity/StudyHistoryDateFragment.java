...
import com.dalread.adapter.StudyHistoryDateAdapter;
...
public class StudyHistoryDateFragment extends BaseVocaHistoryFragment {
    ...
    private StudyHistoryDateAdapter adapter;
    ...
    private void init() {
        adapter = new StudyHistoryDateAdapter(sharedPreferences.getDisplayPronunciation());
            ...
            if (Utils.isConnected(getContext())) {
                ...
                rvHistory.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        application.getDalAiImpl().getStudentStudiedHistoryByDate(
                                String.valueOf(uid),
    ...
    private OnStudyHistoryClickListener onClickListener = new OnStudyHistoryClickListener() {
        ...
        public void onPlayAllClick(Date date) {
            if (activity != null) {
                ArrayList<VocaStudyHistory> h = new ArrayList<>();
                for (VocaStudyHistory history : histories) {
                    if (DateUtils.isSameDate(new Date(DateUtils.secondsToMillis(history.getStudyDateTS())), date)) {
                        h.add(history);
                    }
                }
                Intent intent = new Intent(activity, PlaylistActivity.class);
                intent.putExtra(Constant.BUNDLE.KEY_VOCA_PLAYLIST, h);
