...
import com.dalread.util.AraMultiInAppProductsClass;
...
public class AraMultiPlayerBillingClientHelper extends BillingClientHelper {
    public AraMultiPlayerBillingClientHelper(Activity activity) {
        super(activity);
    }

    public static synchronized BillingClientHelper getInstance(Activity activity) {
        if (billingClientHelper == null) {
            billingClientHelper = new AraMultiPlayerBillingClientHelper(activity);
        }
    ...
    protected InAppProductsClass getInAppProductsClass() {
        return new AraMultiInAppProductsClass();
    }

    @Override
    protected int onUpdatePoints(Purchase purchase) {
        int pointsToAdd = 0;
        Map<String, InAppProductsClass.AraInAppProduct> appProducts = new AraMultiInAppProductsClass().getPointProducts();
