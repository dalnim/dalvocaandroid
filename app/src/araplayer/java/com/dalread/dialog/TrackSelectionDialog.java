...
import com.dalread.adapter.FragmentAdapter;
import com.dalread.listener.OnTrackSelectedListener;
        ...
        trackSelectionDialog.init(
                ...
                /* onClickListener= */ (dialog, which) -> {
                    ...
                    for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
                        ...
                        builder.clearSelectionOverrides(rendererIndex).setRendererDisabled(rendererIndex, isDisable);
                        ...
                        if (trackType == C.TRACK_TYPE_VIDEO || trackType == C.TRACK_TYPE_AUDIO || trackType == C.TRACK_TYPE_TEXT) {
                            TrackSelection trackSelection = new TrackSelection(trackType, isDisable, overrides);
                            trackSelectionList.add(trackSelection);
                        }
                    }
                    trackSelector.setParameters(builder);
                    data = new Gson().toJson(trackSelectionList);
                    onTrackSelectedListener.onSelected(data);
                },
                onDismissListener);
