...
public class MenuFragmentWorkbook extends BaseConvFragment {
    ...
    private void getData() {
        if (parentBook == null) {
            ...
            if (LanguageUtil.isStudyLangEnglish(activity) &&  UserUtil.isDebugOrAdminUser(activity)) {
                //This is temp code. 현재는 모든 단어장 다보게 만듫. 나중에 일부는 인앱으로 해야함.
                vocaBookList.addAll(activity.subDatabase.getServerVocaBookListForEnglishWords());
            }
        } else {
...
//                bindData();
    ...
    private void openVocaListView(VocaBook vocaBook) {
        openNewScreen(
                ConvVocaListActivity.createIntentByBookId(activity, Integer.parseInt(String.valueOf(vocaBook.getIBookId())), true)
        );
    }
