...
### SQLite (MultiPlayerDatabase)

- **위치**: `/Users/dalnimbest/Documents/workspace/dalvocaandroid/app/src/araplayer/java/com/dalread/database/sqlite/MultiPlayerDatabase.java`
- **초기화**: `/Users/dalnimbest/Documents/workspace/dalvocaandroid/app/src/araplayer/java/com/dalread/AraMultiPlayerAssetHelper.java`에서 asset의 `araonesoft.multiplayer.sqlite`를 복사
- **테이블**: DIC_PLAYER_SCREEN, DIC_PLAYER_SCREEN_BACKUP, DIC_PLAYER_SCREEN_AB_REPEAT, DIC_PLAYER_SCREEN_STORED_LAYOUT, DIC_PLAYER_VIDEO_LIST_IN_SCREEN
- **문제**: `/Users/dalnimbest/Documents/workspace/dalvocaandroid/app/src/main/java/com/dalread/database/sqlite/SubDatabaseHelper.java`의 `onCreate()`가 비어 있어 테이블 생성 안 함. asset DB 파일에 의존

### Realm 사용처 (현재 범위: 폐기만)

| 모델 | 주요 사용처 | 처리 |
|------|-------------|------|
| DownloadModel | MultiPlayerDownloadFragment, MainPlayerMediaFragment, PlayerNetworkActivity | **폐기** (현재 작업) |
...
- **Android**: Mac 스키마를 기준으로 하고, Mac에 없는 것만 추가 (video_list_in_screen, speed, rotate 등)
...
DB 파일이 없을 때 앱에서 Mac 스키마로 테이블을 생성한다. MultiPlayerDatabaseHelper에서 수행.

| 단계 | 작업 내용 | 대상 파일/위치 |
|------|-----------|----------------|
| 1.0.1 | MultiPlayerDatabaseHelper 클래스 생성 (SQLiteOpenHelper 상속) | `app/src/araplayer/.../sqlite/MultiPlayerDatabaseHelper.java` (신규) |
| 1.0.2 | onCreate()에 **current_screens** CREATE TABLE (screen_id, file_path, last_time, ab_loop_json, use_ab, resize_mode, volume, speed, rotate — 모두 DEFAULT 지정) | MultiPlayerDatabaseHelper.onCreate() |
| 1.0.3 | onCreate()에 **video_meta** CREATE TABLE (file_path PK, sort_order, file_size, duration, last_time, ab_loop_json, use_ab, resize_mode, speed 등, 모두 DEFAULT) | 동일 |
| 1.0.4 | onCreate()에 **stored_layout** CREATE TABLE (id, name, created_at, grid_row_count, grid_column_count) | 동일 |
| 1.0.5 | onCreate()에 **screens_in_stored_layout** CREATE TABLE (id, layout_id, screen_id, file_path, last_time, ab_loop_json, use_ab, resize_mode, sort_order, speed, rotate, 모두 DEFAULT) | 동일 |
| 1.0.6 | onCreate()에 **app_kv** CREATE TABLE (field1 PK, field2) | 동일 |
| 1.0.7 | onCreate()에 **video_list_in_screen** CREATE TABLE (id, screen_id, file_path) + screen_id 인덱스 | 동일 |
| 1.0.8 | onUpgrade()에서 user_version 또는 버전별 분기 (필요 시) | MultiPlayerDatabaseHelper |
...
| 단계 | 작업 내용 | 대상 파일/위치 |
|------|-----------|----------------|
| 1.0.9 | PLAYER.SQL.TABLE에 current_screens, video_meta, stored_layout, screens_in_stored_layout, app_kv, video_list_in_screen 상수 추가 | Constant.java |
...
| volume | `updateScreenVolumeOnly(screenId, volume)` | current_screens 해당 스크린 행의 volume만 (video_meta에는 volume 없음. Mac 스키마 기준) | 볼륨 변경 시 호출 |
...
| 1.1.13 | (선택) resize_mode 전용: `updateScreenResizeModeOnly`, `updateVideoMetaResizeMode` — 리사이즈 변경 시 전용 함수만 호출 | 동일 |
...
| 단계 | 작업 내용 | 대상 파일/위치 |
|------|-----------|----------------|
| 1.3.1 | PLAYER.SQL.TABLE에 current_screens, video_meta, stored_layout, screens_in_stored_layout, app_kv, video_list_in_screen 상수 추가 (기존 DIC_PLAYER_* 유지 여부는 호환 정책에 따라 결정) | Constant.java |
...
| 1.4.5 | video_list_in_screen 테이블에 대한 insert/delete/query (getAllVideListInScreen 등) | 동일 |
...
1. **Phase 1.0: 테이블 생성** — MultiPlayerDatabaseHelper + onCreate()에 current_screens, video_meta, stored_layout, screens_in_stored_layout, app_kv, video_list_in_screen CREATE TABLE (모든 필드 DEFAULT 지정)
...
4. **Phase 1.1: AB 반복 DB 저장** — video_meta·current_screens 갱신 시 **last_time 전용 함수**·**AB 전용 함수** 분리 (updateScreenLastTimeOnly, updateScreenABLoopOnly, updateVideoMetaLastTime, updateVideoMetaABLoop), ab_loop_json 직렬화, 비디오 열 때 video_meta 참조, loadedLayoutId, 레이아웃 로드, DIC_PLAYER_SCREEN_AB_REPEAT 제거
5. **Phase 1.2~1.8** — stored_layout·screens_in_stored_layout·video_list_in_screen CRUD, Model 보완, 볼륨·재생속도 DB 저장 등
